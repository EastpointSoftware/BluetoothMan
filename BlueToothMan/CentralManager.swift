...
public class CentralManager : NSObject, CBCentralManagerDelegate {
    ...
    public func startScanningTimed(timeoutSeconds:Double, capacity:Int? = nil) -> FutureStream<Peripheral> {
        self.timeoutSeconds = timeoutSeconds
        self._isScanning = true
        self.timeoutScan()
        return self.startScanning(capacity)
    ...
    public func startScanningForServiceUUIDs(timeoutSeconds:Double, uuids:[CBUUID]!, capacity:Int? = nil) -> FutureStream<Peripheral> {
        ...
        self._isScanning = true
        self.timeoutScan()
        return self.startScanningForServiceUUIDs(uuids, capacity:capacity)
    }
    
    
    internal func timeoutScan() {
        Logger.debug("timeout in \(self.timeoutSeconds)s")
        self.centralQueue.delay(self.timeoutSeconds) {
            if self._isScanning {
                if self.peripherals.count == 0 {
                    self.afterPeripheralDiscoveredPromise.failure(BCError.peripheralDiscoveryTimeout)
                }
                self.stopScanning()
            }
        }
